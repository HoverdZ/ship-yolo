"""Static validation for all four PLS-SCAM formal Colab notebooks."""
...
def validate_notebook(path: Path, run_id: str) -> dict:
    ...
    assert "build_pls_scam_config(RUN_ID, run_training=True)" in training
    assert "prepare_pls_scam_experiment(config)" in training
    ...
    assert "update_pls_scam_comparison" in finalization
